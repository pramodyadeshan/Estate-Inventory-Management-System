    ...
    <div class="col-md-10">
        <div class="col-md-12 margin-top-20">
            <div class="d-inline-flex">
                ...
                <div class="p-2"><h2>Expenditure Report</h2></div>
            </div>

            <hr>
            <form action="/filter-date-expend" method="POST" class="report px-20-30">
                @csrf
                <div class="row">
                    <div class="col-md-5">
        ...
        <div class="col-md-12 margin-top-20">
            @if(isset($expends))
            <form action="/download-date-range-expend-PDF" method="POST">
                @csrf
                ...
                <table class="table table-border report-table">
                    ...
                    @if(isset($expends))
                        @foreach($expends as $expend)
                            <tr>
                                <td style="text-align: center">{{$expend->id}}</td>
                                <td class="text-left">{{$expend->date}}</td>
                                <td class="text-left">{{$expend->users->name}}</td>
                                <td class="text-left">{{$expend->note}}</td>
                                <td class="text-right">{{number_format($expend->price,2)}}</td>
                            </tr>
                            @php
